(() => {
  ...
  function renderRedistributionHeroStat(root){
    ...
    const pct=(market.value-disposable.value)/market.value*100;
    root.innerHTML=`<div class="redistribution-hero-stat"><strong>−${fmt(pct)} %</strong><span>${lang==="en"?"lower measured Gini after taxes and transfers":"nižší naměřený Giniho koeficient po daních a transferech"}<small>${esc(name(code))} · ${market.year}${disposable.year!==market.year?` / ${disposable.year}`:""}</small></span></div>`;
  }

  function renderRedistributionSummary(root){
    const market=country()?.distribution?.market_gini,disposable=country()?.distribution?.disposable_gini;
    if(!market||!disposable){empty(root);return;}
    const reduction=market.value-disposable.value,pct=reduction/market.value*100,poverty=obs(code,"poverty_rate");
    const en=lang==="en";
    root.innerHTML=`<div class="redistribution-summary"><div><span class="summary-eyebrow">${esc(name(code))} · OECD</span><h3>${en?`Measured income inequality is ${fmt(pct)}% lower after taxes and transfers.`:`Naměřená příjmová nerovnost je po daních a transferech o ${fmt(pct)} % nižší.`}</h3><p>${en?"This is an accounting comparison of two income distributions, not an estimate of any individual programme’s effect.":"Jde o účetní srovnání dvou rozdělení příjmů, nikoli o odhad účinku jednotlivého programu."}</p></div><div><span class="summary-eyebrow">${esc(t().reduction)}</span><strong class="summary-value">−${fmt(reduction,3)}</strong><span class="summary-metric">Gini: ${fmt(market.value,3)} → ${fmt(disposable.value,3)}</span><small class="summary-years">${en?"Before":"Před"} ${market.year} · ${en?"After":"Po"} ${disposable.year}${poverty?`<br>${en?"Relative poverty":"Relativní chudoba"}: ${fmt(poverty.value)} % · ${poverty.year}`:""}</small></div></div>`;
  }

  function renderBridge(root){
    const market=country()?.distribution?.market_gini, disposable=country()?.distribution?.disposable_gini;
    if(!market||!disposable){empty(root);return;}
    const reduction=market.value-disposable.value,pct=reduction/market.value*100;
    root.innerHTML=`<section class="oecd-chart-block">${header("OECD Income Distribution Database",lang==="en"?"Inequality before and after taxes and transfers":"Nerovnost před daněmi a transfery a po nich",lang==="en"?"Both bars use the same 0–1 Gini scale. The difference is descriptive, not a programme-effect estimate.":"Oba pruhy mají stejnou škálu Giniho koeficientu 0–1. Rozdíl je popisný, nikoli odhad účinku programu.")}<div class="oecd-chart-canvas"><div class="oecd-bridge"><div class="oecd-bridge-column"><span>${esc(t().market)}</span><div class="oecd-bridge-track" aria-hidden="true"><i style="--width:${market.value*100}%"></i></div><strong>${fmt(market.value,3)}</strong><small>${market.year}</small></div><div class="oecd-bridge-change"><span>${esc(t().reduction)}</span><strong>−${fmt(reduction,3)}</strong><small>−${fmt(pct)} %</small></div><div class="oecd-bridge-column"><span>${esc(t().disposable)}</span><div class="oecd-bridge-track" aria-hidden="true"><i style="--width:${disposable.value*100}%"></i></div><strong>${fmt(disposable.value,3)}</strong><small>${disposable.year}</small></div></div></div><p class="oecd-chart-note">${esc(data.metrics.market_gini[`boundary_${lang}`])} ${esc(data.metrics.disposable_gini[`boundary_${lang}`])}</p></section>`;
  }
  ...
  function renderSocx(root){
    ...
    const title=lang==="en"?"Social spending by programme":"Sociální výdaje podle programu";
    const label=`${title}: ${entries.map(x=>`${x.meta[lang==="en"?1:0]} ${fmt(x.obs.value)} %`).join(", ")}; ${t().total} ${fmt(social.total.value)} %`;
    const gdp=lang==="en"?"GDP":"HDP";
    root.innerHTML=`<section class="oecd-chart-block">${header("OECD SOCX",title,lang==="en"?"Each programme uses its latest available year. The separate aggregate may be newer and must not be read as the sum of these components.":"Každý program používá poslední dostupný rok. Samostatný souhrn může být novější a není součtem zde uvedených složek.")}<div class="oecd-chart-canvas"><div class="oecd-stack" role="img" aria-label="${esc(label)}">${entries.map(x=>`<i style="width:${x.obs.value/sum*100}%;background:${x.meta[2]};color:${["#a8b63f", "#8b8d83", "#8e7d68"].includes(x.meta[2]) ? "#171918" : "#faf7ef"}" title="${esc(x.meta[lang==="en"?1:0])}: ${fmt(x.obs.value)}% ${gdp} · ${x.obs.year}">${x.obs.value/sum>0.1?fmt(x.obs.value):""}</i>`).join("")}</div><ul class="oecd-stack-legend">${entries.map(x=>`<li><i style="--color:${x.meta[2]}"></i><span>${esc(x.meta[lang==="en"?1:0])} <small>· ${x.obs.year}</small></span><b>${fmt(x.obs.value)} % ${gdp}</b></li>`).join("")}</ul><div class="oecd-chart-callouts"><article><span>${lang==="en"?"Separate aggregate":"Samostatný souhrn"}</span><strong>${fmt(social.total.value)} % ${gdp}</strong><small>${social.total.year} · ${esc(t().publicSocial)}</small></article></div></div><p class="oecd-chart-note">${esc(data.metrics.social_spending[`boundary_${lang}`])}</p></section>`;
  }
  ...
  function renderAutonomy(root){
    ...
    root.innerHTML=`<section class="oecd-chart-block">${header("OECD Fiscal Decentralisation Database",lang==="en"?"Local control over local taxes":"Rozhodovací pravomoc nad místními daněmi",lang==="en"?"The spectrum separates discretion over the tax rate or base, shared taxes, and taxes effectively set by central government. It measures authority, not revenue size.":"Spektrum odděluje rozhodování o sazbě či základu, sdílené daně a daně fakticky určené centrem. Měří pravomoc, nikoli velikost příjmů.")}<div class="oecd-chart-canvas"><div class="oecd-spectrum">${rows.map(x=>`<div class="oecd-spectrum-row${x.code===code?" is-selected":""}" data-oecd-spectrum-code="${x.code}" tabindex="0" role="img" aria-label="${esc(name(x.code))}"><span>${esc(name(x.code))}</span><div class="oecd-spectrum-track" aria-hidden="true"><i style="width:${x.row.autonomous_share_pct}%"></i><i style="width:${x.row.shared_share_pct}%"></i><i style="width:${x.row.other_or_central_share_pct}%"></i></div><b>${fmt(x.row.autonomous_share_pct)} %</b></div>`).join("")}</div><div class="oecd-spectrum-key"><span><i></i>${esc(t().autonomous)}</span><span><i></i>${esc(t().shared)}</span><span><i></i>${esc(t().central)}</span></div><div class="oecd-hover-tooltip" role="tooltip" hidden></div></div><p class="oecd-chart-note">${esc(data.metrics.local_tax_autonomy[`boundary_${lang}`])}</p></section>`;
    const tooltip=root.querySelector(".oecd-hover-tooltip");
    const hide=()=>{tooltip.hidden=true;root.querySelectorAll(".oecd-spectrum-row.is-hovered").forEach(row=>row.classList.remove("is-hovered"));};
    const show=(countryCode,clientX,clientY)=>{const item=rows.find(entry=>entry.code===countryCode);if(!item)return;tooltip.innerHTML=`<strong>${esc(name(item.code))}</strong><small>${esc(t().year)} ${item.row.year}</small><div><span>${esc(t().autonomous)}</span><b>${fmt(item.row.autonomous_share_pct)} %</b></div><div><span>${esc(t().shared)}</span><b>${fmt(item.row.shared_share_pct)} %</b></div><div><span>${esc(t().central)}</span><b>${fmt(item.row.other_or_central_share_pct)} %</b></div>`;tooltip.hidden=false;root.querySelectorAll(".oecd-spectrum-row").forEach(row=>row.classList.toggle("is-hovered",row.dataset.oecdSpectrumCode===countryCode));const gap=15,pad=12,rect=tooltip.getBoundingClientRect();let left=clientX+gap,top=clientY+gap;if(left+rect.width>innerWidth-pad)left=clientX-rect.width-gap;if(top+rect.height>innerHeight-pad)top=clientY-rect.height-gap;tooltip.style.left=`${Math.max(pad,left)}px`;tooltip.style.top=`${Math.max(pad,top)}px`;};
    const spectrum=root.querySelector(".oecd-spectrum");
    spectrum.addEventListener("pointermove",event=>{const row=event.target.closest("[data-oecd-spectrum-code]");if(row)show(row.dataset.oecdSpectrumCode,event.clientX,event.clientY);else hide();});
    spectrum.addEventListener("pointerleave",hide);
    spectrum.addEventListener("focusin",event=>{const row=event.target.closest("[data-oecd-spectrum-code]");if(!row)return;const rect=row.getBoundingClientRect();show(row.dataset.oecdSpectrumCode,Math.min(innerWidth-24,rect.right),rect.top);});
    spectrum.addEventListener("focusout",hide);
  }
  ...
  function scatterSvg(xKey,yKey,selected=code){
    ...
    const xl=data.metrics[xKey]?.[`label_${lang}`]||xKey,yl=data.metrics[yKey]?.[`label_${lang}`]||yKey;
    return `<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="${esc(xl)} × ${esc(yl)}">${grids}${points.map(p=>`<circle class="${p.c===selected?"is-selected":""}" cx="${sx(p.x.value)}" cy="${sy(p.y.value)}" r="${p.c===selected?7:5}"><title>${esc(name(p.c))}: ${fmt(p.x.value)} / ${fmt(p.y.value)}</title></circle><text class="country" x="${sx(p.x.value)+8}" y="${sy(p.y.value)-7}">${p.c}</text>`).join("")}<text class="axis-title" x="${W/2}" y="${H-8}" text-anchor="middle">${esc(xl)}</text><text class="axis-title" transform="translate(16 ${H/2}) rotate(-90)" text-anchor="middle">${esc(yl)}</text></svg>`;
  }
  ...
  function renderRedistributionComparison(root){
    const codes=(root.dataset.countryCodes||"").split(",").filter(Boolean);
    const rows=codes.map(c=>({c,market:data.countries[c]?.distribution?.market_gini,disposable:data.countries[c]?.distribution?.disposable_gini,poverty:obs(c,"poverty_rate")})).filter(row=>row.market||row.disposable||row.poverty).sort((a,b)=>(a.disposable?.value??Infinity)-(b.disposable?.value??Infinity));
    if(!rows.length){empty(root);return;}
    const en=lang==="en";
    const headings=en?["Country","Before taxes and transfers · Gini","After taxes and transfers · Gini","Difference · Gini points","Relative poverty · below 50% of median"]:["Země","Před daněmi a transfery · Gini","Po daních a transferech · Gini","Rozdíl · body Giniho koeficientu","Relativní chudoba · pod 50 % mediánu"];
    const value=(item,digits=3)=>item?`${fmt(item.value,digits)} <small>${item.year}</small>`:"—";
    root.innerHTML=`<section class="oecd-chart-block">${header("OECD Income Distribution Database",en?"Income distribution across countries":"Rozdělení příjmů napříč zeměmi",en?"Sorted by after-tax inequality. Figures refer to each country’s latest available year; compare definitions and years before drawing conclusions.":"Seřazeno podle nerovnosti po zdanění. Údaje jsou z posledního dostupného roku každé země; před závěry zkontrolujte definice a roky.")}${canvas(en?"Income distribution comparison":"Srovnání rozdělení příjmů","oecd-outcomes-wrap")}<table class="oecd-data-table oecd-comparison-table"><thead><tr>${headings.map(label=>`<th scope="col">${esc(label)}</th>`).join("")}</tr></thead><tbody>${rows.map(row=>{const gap=row.market&&row.disposable?row.market.value-row.disposable.value:null;return `<tr class="${row.c===code?"is-selected":""}"><th scope="row">${esc(name(row.c))}</th><td>${value(row.market)}</td><td>${value(row.disposable)}</td><td>${gap===null?"—":`−${fmt(gap,3)}`}</td><td>${row.poverty?`${fmt(row.poverty.value)} % <small>${row.poverty.year}</small>`:"—"}</td></tr>`}).join("")}</tbody></table></div><p class="oecd-chart-note">${en?"The before/after gap is descriptive, not a ranking of programme effectiveness.":"Rozdíl před a po je popisný, nikoli pořadí účinnosti programů."}</p></section>`;
  }
  ...
  function renderOutcomes(root){
    ...
    const title=lang==="en"?"Outcomes in their own units":"Výsledky v původních jednotkách";
    root.innerHTML=`<section class="oecd-chart-block">${header("OECD outcomes",title,lang==="en"?"Each outcome stays in its own unit and year; the table does not merge them into a single score.":"Každý výsledek zůstává ve své jednotce a roce; do jednoho skóre je neslučujeme.")}${canvas(title,"oecd-outcomes-wrap")}<table class="oecd-data-table"><thead><tr><th>${lang==="en"?"Country":"Země"}</th>${keys.map(k=>`<th>${esc(data.metrics[k]?.[`label_${lang}`]||k)}</th>`).join("")}</tr></thead><tbody>${Object.keys(data.countries).map(c=>`<tr class="${c===code?"is-selected":""}"><td>${esc(name(c))}</td>${keys.map(k=>{const o=obs(c,k),unit=data.metrics[k]?.[`unit_${lang}`]||"";return `<td>${o?`${fmt(o.value,k.includes("gini")?3:1)} ${esc(unit)} <small>${o.year}</small>`:"—"}</td>`}).join("")}</tr>`).join("")}</tbody></table></div></section>`;
  }
  ...
  const RENDERERS={tax_wedge:renderTaxWedge,redistribution_hero_stat:renderRedistributionHeroStat,redistribution_summary:renderRedistributionSummary,redistribution_bridge:renderBridge,redistribution_comparison:renderRedistributionComparison,socx_composition:renderSocx,pension_curve:renderPension,autonomy_spectrum:renderAutonomy,tax_matrix:renderTaxMatrix,employee_taxes:renderEmployeeTaxes,corporate_rates:renderCorporate,carbon_autonomy:renderCarbonAutonomy,page_scatter:renderPageScatter,outcomes_table:renderOutcomes,deep_kpis:renderDeepKpis,source_grid:renderSources};
