(() => {
  ...
  function renderSocx(root){
    ...
    const title=lang==="en"?"The composition of public social spending":"Složení veřejných sociálních výdajů";
    const label=`${title}: ${entries.map(x=>`${x.meta[lang==="en"?1:0]} ${fmt(x.obs.value)} %`).join(", ")}; ${t().total} ${fmt(social.total.value)} %`;
    root.innerHTML=`<section class="oecd-chart-block">${header("OECD SOCX",title,lang==="en"?"Programme values use their latest available observations. The total can have a newer year, so components are not forced to reconcile to it.":"Programy používají svůj poslední dostupný rok. Celkový údaj může být novější, proto složky násilně nesrovnáváme s celkem.")}<div class="oecd-chart-canvas"><div class="oecd-stack" role="img" aria-label="${esc(label)}">${entries.map(x=>`<i style="width:${x.obs.value/sum*100}%;background:${x.meta[2]};color:${["#a8b63f", "#8b8d83", "#8e7d68"].includes(x.meta[2]) ? "#171918" : "#faf7ef"}" title="${esc(x.meta[lang==="en"?1:0])}: ${fmt(x.obs.value)}% HDP">${x.obs.value/sum>0.1?fmt(x.obs.value):""}</i>`).join("")}</div><ul class="oecd-stack-legend">${entries.map(x=>`<li><i style="--color:${x.meta[2]}"></i><span>${esc(x.meta[lang==="en"?1:0])} <small>· ${x.obs.year}</small></span><b>${fmt(x.obs.value)} %</b></li>`).join("")}</ul><div class="oecd-chart-callouts"><article><span>${esc(t().total)}</span><strong>${fmt(social.total.value)} % HDP</strong><small>${social.total.year} · ${esc(t().publicSocial)}</small></article></div></div><p class="oecd-chart-note">${esc(data.metrics.social_spending[`boundary_${lang}`])}</p></section>`;
  }
