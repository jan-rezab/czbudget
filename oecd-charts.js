(() => {
  ...
  function renderSocx(root){
    ...
    root.innerHTML=`<section class="oecd-chart-block">${header("OECD SOCX",lang==="en"?"The composition of public social spending":"Složení veřejných sociálních výdajů",lang==="en"?"Programme values use their latest available observations. The total can have a newer year, so components are not forced to reconcile to it.":"Programy používají svůj poslední dostupný rok. Celkový údaj může být novější, proto složky násilně nesrovnáváme s celkem.")}<div class="oecd-chart-canvas"><div class="oecd-stack" role="img">${entries.map(x=>`<i style="width:${x.obs.value/sum*100}%;background:${x.meta[2]}" title="${esc(x.meta[lang==="en"?1:0])}: ${fmt(x.obs.value)}% HDP">${x.obs.value/sum>0.1?fmt(x.obs.value):""}</i>`).join("")}</div><ul class="oecd-stack-legend">${entries.map(x=>`<li><i style="--color:${x.meta[2]}"></i><span>${esc(x.meta[lang==="en"?1:0])} <small>· ${x.obs.year}</small></span><b>${fmt(x.obs.value)} %</b></li>`).join("")}</ul><div class="oecd-chart-callouts"><article><span>${esc(t().total)}</span><strong>${fmt(social.total.value)} % HDP</strong><small>${social.total.year} · ${esc(t().publicSocial)}</small></article></div></div><p class="oecd-chart-note">${esc(data.metrics.social_spending[`boundary_${lang}`])}</p></section>`;
  }
  ...
  function renderOutcomes(root){
    ...
    root.innerHTML=`<section class="oecd-chart-block">${header("OECD outcomes",lang==="en"?"Outcomes in their own units":"Výsledky v původních jednotkách",lang==="en"?"Each outcome stays in its own unit and year; the table does not merge them into a single score.":"Každý výsledek zůstává ve své jednotce a roce; do jednoho skóre je neslučujeme.")}<div class="oecd-chart-canvas oecd-outcomes-wrap"><table class="oecd-data-table"><thead><tr><th>${lang==="en"?"Country":"Země"}</th>${keys.map(k=>`<th>${esc(data.metrics[k]?.[`label_${lang}`]||k)}</th>`).join("")}</tr></thead><tbody>${Object.keys(data.countries).map(c=>`<tr class="${c===code?"is-selected":""}"><td>${esc(name(c))}</td>${keys.map(k=>{const o=obs(c,k),unit=data.metrics[k]?.[`unit_${lang}`]||"";return `<td>${o?`${fmt(o.value,k.includes("gini")?3:1)} ${esc(unit)} <small>${o.year}</small>`:"—"}</td>`}).join("")}</tr>`).join("")}</tbody></table></div></section>`;
  }
