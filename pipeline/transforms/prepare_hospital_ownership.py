...
# Imported from the unversioned sibling scripts/ directory. The workspace root
# is resolved the same way as every other transform here: CZBUDGET_WORKSPACE_ROOT
# when set, otherwise three levels up from this file
# (website/pipeline/transforms -> workspace root). The previous
# `.parent.parent` walked to the wrong directory once the file moved.
ROOT = pathlib.Path(os.environ.get("CZBUDGET_WORKSPACE_ROOT", pathlib.Path(__file__).resolve().parents[3]))
COUNTRIES = ROOT / "website" / "data" / "countries"
OUT = pathlib.Path(os.environ.get("HOSPITAL_OWNERSHIP_OUT", ROOT / "outputs" / "20260826-hospital-ownership"))

# The build stamp was a frozen literal, so every rerun re-published the same
# claim about its own freshness. CZBUDGET_GENERATED_AT pins it for a
# reproducible build; otherwise it is the real generation date.
GENERATED_AT = (os.environ.get("CZBUDGET_GENERATED_AT") or datetime.now(timezone.utc).isoformat())[:10]
...
def main() -> None:
    ...
    cz_health = json.loads(
        (ROOT / "website" / "data" / "cz-health-budget.v1.json").read_text(encoding="utf-8")
    )
