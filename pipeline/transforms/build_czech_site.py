...
def head(title: str, description: str, canonical_path: str, depth: int, schema: dict, language: str = "cs") -> str:
  ...
  <script src="{root}global-nav.js?v=20260824-identity-outlines" defer></script>
  <meta charset="utf-8">
...
<main id="top">
  ...
  <section class="detail-analysis" id="rozpocet">
    ...
    <div class="detail-section-title"><div><span class="kicker">02 / Struktura toků</span><h2>Odkud a kam.</h2></div><p>Příjmové a výdajové třídy podle rozpočtové skladby.</p></div>
    ...
    <div class="detail-section-title"><div><span class="kicker">04 / Benchmark</span><h2>Pozice v kohortě.</h2></div><p>Percentil ukazuje relativní výši ukazatele; sám o sobě není známkou kvality hospodaření.</p></div>
    <article class="detail-panel benchmark-panel">{''.join(benchmark_rows)}</article>
  </section>
  <section class="data-contract" id="metodika"><div><span class="kicker">05 / Data a metodika</span><h2>Auditovatelný detail.</h2><p>Údaje jsou vedeny na úrovni samostatné účetní jednotky. Příspěvkové organizace ani samostatné městské části nejsou přičteny podruhé.</p></div><div class="source-list"><a href="{esc(entity['sources']['budget'])}" rel="noopener" target="_blank"><span>Rozpočet</span><strong>Monitor státní pokladny ↗</strong></a><a href="{esc(entity['sources']['entity'])}" rel="noopener" target="_blank"><span>Identita subjektu</span><strong>ARES ↗</strong></a><a href="../../../data/entities/{esc(entity['national_id'])}.json"><span>Strojová data</span><strong>JSON · schema {esc(data['schema_version'])} ↗</strong></a></div></section>
</main>
{footer('../../../')}
