...
root = here.parents[1]
with tempfile.TemporaryDirectory(prefix="taiwan-city-budget-") as temp:
    target = Path(temp)
    for name in ("worker.py", "requirements.txt", "cloudbuild.yaml", "source.csv"):
        shutil.copy2(here / name, target / name)
    shutil.copy2(root / "pipeline/config/taiwan_city_budget_sources.json", target / "config.json")
    subprocess.run([
