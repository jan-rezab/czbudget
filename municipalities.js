(() => {
  ...
  function setLanguage() {
    ...
    if(state.data){
      ...
      $(".municipal-hero h1").textContent=state.lang==="en"?(state.data.countries.length===10?"Municipal budgets in ten countries":`Municipal budgets in ${count} countries`):(state.data.countries.length===10?"Rozpočty obcí v deseti zemích":`Rozpočty obcí v ${count} zemích`);
    }
