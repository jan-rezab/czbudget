(() => {
  ...
  function setLanguage() {
    ...
    document.querySelector('[data-view-link="czechia"]')?.setAttribute("href", `czechia/?lang=${state.lang}`);
  }
  function countryCards() {
    ...
    $("#country-grid").innerHTML = state.data.countries.map((country) => {
      const cta = country.code === "CZE" ? `<a href="${assetRoot}municipalities/czechia/?lang=${state.lang}">${esc(t().openCzechia)}</a>` : `<button type="button" data-filter-country="${country.code}">${esc(t().entities)} →</button>`;
