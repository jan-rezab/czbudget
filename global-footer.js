(() => {
  ...
  function render() {
    ...
    footer.innerHTML = `<a class="footer-brand" href="${href("index.html")}" aria-label="Public Spending Data"><img src="${assetRoot}assets/logo-lockup-dark.svg" width="190" height="48" alt="Public Spending Data"></a><address class="footer-address">${t.address}</address><a class="footer-hlidac" href="https://www.hlidacstatu.cz/" target="_blank" rel="noopener" aria-label="Hlídač státu, z. ú."><img src="${assetRoot}assets/hlidac-statu-horizontal-inverted-bw.svg" width="220" height="35" alt="Hlídač státu"></a><nav class="footer-links" aria-label="${lang === "en" ? "Project information" : "Informace o projektu"}"><a href="${href("about.html")}">${t.about}</a><a href="${href("methodology.html")}">${t.methodology}</a></nav>`;
  }
