(() => {
  ...
  const coverageCategories=[
    ...
    {id:"budgetDetail",cs:"Položkové rozpočty obcí",en:"Itemized municipal budgets",modules:["municipalities"],municipal:true}
  ];
  let current = lang === "en" ? "en" : "cs";
  let methodologyData=null,qualityData=null,releaseData=null,municipalityData=null,itemizedBudgetData=null,transportPerformanceData=null,transportBudgetData=null,activeCoverageNode=null;
  ...
  function renderDataHealth(){
    ...
    const artifacts=releaseData?.artifacts||[],publishedBytes=artifacts.reduce((sum,artifact)=>sum+(Number(artifact.bytes)||0),0),publishedFiles=artifacts.reduce((sum,artifact)=>sum+(Number(artifact.files)||1),0),municipalRecords=Number(qualityData?.counts?.municipal_history_records)||0;
    const total=document.querySelector("#status-data-total");if(total)total.textContent=publishedBytes?`${new Intl.NumberFormat(current==="cs"?"cs-CZ":"en-GB",{maximumFractionDigits:1}).format(publishedBytes/1e6)} MB`:"—";
    const meta=document.querySelector("#status-meta");if(meta)meta.textContent=`${integer(publishedFiles)} ${sc.publishedFiles} · ${integer(municipalRecords)} ${sc.municipalRecords}`;
  }
  function render() {
  ...
  ]).then(([methodology,quality,release,municipalities,itemizedBudgets,transportPerformance,transportBudget])=>{methodologyData=methodology;qualityData=quality;releaseData=release;municipalityData=municipalities;itemizedBudgetData=itemizedBudgets;transportPerformanceData=transportPerformance;transportBudgetData=transportBudget;renderMethodology();renderDataHealth();}).catch(error=>{const summary=document.querySelector("#method-ledger-summary");if(summary)summary.textContent=`Methodology data: ${error}`;const health=document.querySelector("#data-health-root");if(health)health.textContent=`Data health: ${error}`;});
})();
