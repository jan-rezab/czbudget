(() => {
  ...
  const copy = {
    cs: {
      ...
      atlasEyebrow:"Obecní rozpočty · svět",atlasTitle:"Co země zveřejňují — a co v národní vrstvě chybí.",atlasLead:"Velká mapa rozlišuje celý rozpočtový životní cyklus, dvojici plán–skutečnost, samotné závěrečné účty a země bez jednotného národního položkového datasetu.",
    ...
    en: {
      ...
      atlasEyebrow:"Municipal budgets · world",atlasTitle:"What countries publish — and what is missing from the national layer.",atlasLead:"The large map distinguishes the full budget lifecycle, paired budget and actual data, final accounts alone, and countries without a single national item-level dataset.",
  ...
  function renderDataHealth(){
    ...
    root.innerHTML=`<div class="data-health-status"><span class="data-health-pass">● ${esc(sc.checksPassed)}</span><span>${esc(sc.snapshot)}: <b>${esc(generatedLabel)}</b></span></div><div class="data-health-kpis">${cards.map(([label,value,note])=>`<article><span>${esc(label)}</span><strong>${integer(value)}</strong><small>${esc(note)}</small></article>`).join("")}</div><div class="data-health-downloads"><a href="data/international-municipalities.v1.json" download>${current === "en" ? "Municipal" : "Obecní"} JSON ↓</a><a href="data/methodology-sources.v1.json" download>${esc(sc.ledgerJson)}</a><a href="data/data-quality-report.v1.json" download>QA JSON ↓</a></div>`;
