(() => {
  ...
  render();
  if(document.body.dataset.page==="methodology")fetch("data/methodology-sources.v1.json").then(response=>{if(!response.ok)throw new Error(response.status);return response.json();}).then(data=>{methodologyData=data;renderMethodology();}).catch(error=>{const summary=document.querySelector("#method-ledger-summary");if(summary)summary.textContent=`Methodology data: ${error}`;});
})();
