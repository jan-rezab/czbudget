(() => {
  ...
  Object.assign(statusCopy.en,{
    ...
    act2Title:"How current it is",act2Lead:"The same countries and layers as part 1, but the cell measures the latest fiscal, reporting or observation year and whether it is an actual, a plan, an estimate or a projection.",
  ...
  const coverageCategories=[
    ...
    {id:"municipalities",cs:"Obce · adresář / souhrny",en:"Municipalities · directory / headlines",modules:["municipalities"],municipal:true},
    {id:"municipalHistory",cs:"Obce · časová řada",en:"Municipalities · time series",modules:["municipalities"],municipal:true},
  ...
  Object.assign(statusCopy.cs,{searchCountry:"Hledat zemi",searchCountryPlaceholder:"Název nebo kód země",loading:"Načítám…",loadingPublishedMap:"Načítám mapu publikovaného pokrytí…",loadingAccounting:"Načítám rozsah pokrytí…",selectFreshnessCell:"Vyberte buňku pro detail vrstvy.",loadingSources:"Načítám zdrojovou tabulku…",loadingAtlas:"Načítám atlas…"});
  Object.assign(statusCopy.en,{searchCountry:"Search countries",searchCountryPlaceholder:"Country name or code",loading:"Loading…",loadingPublishedMap:"Loading the published coverage map…",loadingAccounting:"Loading accounting coverage…",selectFreshnessCell:"Select a cell to inspect this layer.",loadingSources:"Loading the source registry…",loadingAtlas:"Loading the atlas…"});
  const esc=value=>String(value??"").replace(/[&<>'"]/g,char=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"}[char]));
  const statusLabel=status=>copy[current][({full:"ledgerFull",partial:"ledgerPartial",aggregate:"ledgerAggregate",not_loaded:"ledgerNotLoaded",unavailable:"ledgerUnavailable"})[status]||"ledgerUnavailable"];
  const availabilityLabel=status=>({loaded:statusCopy[current].loadedSource,source_available:statusCopy[current].sourceExists,fragmented:statusCopy[current].fragmented,not_found:statusCopy[current].notFound,not_researched:statusCopy[current].notResearched})[status]||statusCopy[current].notResearched;
  ...
  function researchedGap(code,category){
    ...
    if(!research)return {primary:"—",secondary:sc.notResearched,status:"not-researched"};
  ...
  function renderCoverageMatrix(){
    ...
    const score=country=>coverageCategories.reduce((total,category)=>total+(coverageCell(country.code,category).status!=="missing"?1:0),0);
    const visibleCountries=statusCountries().filter(country=>!coverageCountryQuery||fold(`${country[current]} ${country.code}`).includes(fold(coverageCountryQuery))).sort((a,b)=>score(b)-score(a)||String(a[current]).localeCompare(String(b[current]),current));
    if(body)body.innerHTML=visibleCountries.map(country=>{
