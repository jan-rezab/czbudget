(() => {
  ...
  const copy = {
    en: {
      mode:"Coverage layer", readiness:"PSD composite source readiness", national:"OBS central-government score", municipal:"Municipal item-level lifecycle",
      ...
      caveat:"The PSD composite source readiness combines the OBS central-government score with up to 20 points for verified municipal depth. Missing municipal research adds no bonus and remains provisional. Dark gray means not researched or not scored—never “publishes nothing.” The tan colour is the middle band.",
      ...
      country:"Country", psdScore:"PSD composite source readiness", obsScore:"OBS central-government score", municipalColumn:"Municipal layer", loadColumn:"PSD coverage", loadedProfile:"Loaded", crawlStarted:"Crawl started", notLoaded:"Not loaded", stages:["Approved","Revised","In-year","Final","Function","Economic","API / bulk"], outside:"Outside the 195-state universe",
      ...
      scoreWhy:"Why this score", obs:"OBS central government", municipalCapability:"municipal capability", municipalBonus:"municipal bonus", noObs:"OBS did not assess this country", noMunicipal:"municipal layer not yet researched", completeEvidence:"central and municipal evidence", nationalOnly:"central evidence; municipal review pending", municipalOnly:"municipal evidence; no OBS score", noEvidence:"no verified score component", provisionalNote:"† The index is the OBS central-government score alone: the municipal layer has not been researched, so it adds no bonus. A dash in the score column means no OBS survey exists, not that a government publishes nothing."
    },
    cs: {
      mode:"Vrstva pokrytí", readiness:"Složené hodnocení dostupnosti zdrojů PSD", national:"Skóre OBS pro centrální vládu", municipal:"Položkový životní cyklus obcí",
      ...
      scoreWhy:"Proč má toto skóre", obs:"OBS centrální vláda", municipalCapability:"obecní schopnosti", municipalBonus:"obecní bonus", noObs:"OBS tuto zemi neposuzoval", noMunicipal:"obecní vrstva zatím neprověřena", completeEvidence:"státní i obecní důkazy", nationalOnly:"státní důkazy; obecní průzkum čeká", municipalOnly:"obecní důkazy; bez skóre OBS", noEvidence:"bez ověřené složky skóre", provisionalNote:"† Index je pouze skóre OBS pro ústřední vládu: obecní vrstva nebyla prověřena, a tak nepřidává žádný bonus. Pomlčka ve sloupci skóre znamená, že chybí průzkum OBS, nikoli že vláda nic nezveřejňuje."
    }
  };
  ...
  function evidenceTable(t,countries) {
    const rows = sortedCountries(countries).map((country) => {
      ...
      return `<tr id="atlas-row-${country.iso2}"><th><span class="atlas-flag" aria-hidden="true">${flag(country.iso2)}</span><span>${esc(country[`name_${language()}`])}<small>${esc(country.iso2.toUpperCase())}</small></span></th><td class="atlas-score ${country.budget_transparency_index.band} atlas-evidence-${country.budget_transparency_index.evidence_status}" title="${esc(evidenceLabel(country.budget_transparency_index.evidence_status,t))}" aria-label="${esc(`${country.budget_transparency_index.score ?? t.notResearched} — ${evidenceLabel(country.budget_transparency_index.evidence_status,t)}`)}">${country.budget_transparency_index.score ?? "●"}</td><td class="atlas-score ${country.national_budget.band}">${country.national_budget.score ?? "●"}</td><td class="atlas-municipal-status atlas-${municipal.category}">${esc(municipalText)}</td><td class="atlas-load atlas-load-${country.psd_coverage.ingestion_status}">${esc(loadText)}</td>${featureKeys.map((key) => `<td class="${symbolClass(country,key)}" aria-label="${esc(({available:t.available,unavailable:t.unavailable,verify:t.verify,not_researched:t.notResearched})[featureCategory(country,key)])}">${symbol(country,key)}</td>`).join("")}</tr>`;
    }).join("");
