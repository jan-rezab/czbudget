...
function fillBenchmarkSelects(){$("#metric-select").innerHTML=sovereignData.metrics.filter(m=>["revenue_pct_gdp","expenditure_pct_gdp","balance_pct_gdp","primary_balance_pct_gdp","gross_debt_pct_gdp","real_gdp_growth_pct","inflation_pct","unemployment_pct"].includes(m.metric_code)).map(m=>`<option value="${m.metric_code}" ${m.metric_code===benchmarkState.metric?"selected":""}>${esc(metricName(m))}</option>`).join("");$("#country-select").innerHTML=sovereignData.countries.map(c=>`<option value="${c.country_code}" ${c.country_code===benchmarkState.country?"selected":""}>${esc(countryName(c))}</option>`).join("");}
