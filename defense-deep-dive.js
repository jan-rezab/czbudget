(() => {
  ...
  function renderComparison(){
    const commonYears=data.countries.map(item=>new Set(item.comparison.series.filter(row=>Number.isFinite(row[1])).map(row=>row[0])))
      .reduce((shared,years)=>new Set([...shared].filter(year=>years.has(year))));
    const year=commonYears.size?Math.max(...commonYears):null;
    const comparison=data.countries.map(item=>({...item,comparison:{...item.comparison,latest:year===null?null:{year,value:item.comparison.series.find(row=>row[0]===year)?.[1]}}}));
    const nato=comparison.filter(item=>item.nato_member).sort((a,b)=>(a.code==="USA"?-1:b.code==="USA"?1:(b.comparison.latest?.value||0)-(a.comparison.latest?.value||0)));
    const others=comparison.filter(item=>!item.nato_member).sort((a,b)=>(b.comparison.latest?.value||0)-(a.comparison.latest?.value||0));
    $("#comparison-year").textContent=year??"—";
  ...
  function renderCountry(){
    ...
    const comparisonSource=item.comparison.source||data.comparison_source;
    $("#comparison-source").href=comparisonSource.url;
    $("#comparison-source").textContent=comparisonSource.title;
    ...
    if(!definitionNote){definitionNote=document.createElement("p");definitionNote.id="definition-note";$("#coverage-note").after(definitionNote)}
    const detail=item.definition_detail;
    definitionNote.textContent=detail?(lang==="en"
      ?`Separate measures: NATO core defence ${detail.nato_latest.year} estimate ${number(detail.nato_latest.total_current_million_czk/1000,2)} CZK bn; chapter 307 reported outturn ${detail.chapter_latest.year} ${number(detail.chapter_latest.total/1e6,2)} CZK bn. Their definitions and budget stages differ.`
      :`Samostatné ukazatele: odhad jádrových výdajů NATO ${detail.nato_latest.year} ${number(detail.nato_latest.total_current_million_czk/1000,2)} mld. Kč; vykázaná skutečnost kapitoly 307 za ${detail.chapter_latest.year} ${number(detail.chapter_latest.total/1e6,2)} mld. Kč. Definice i rozpočtová fáze se liší.`):"";
    definitionNote.hidden=!detail;
    ...
    if(detail) $("#national-sources").insertAdjacentHTML("beforeend",`<a href="../../${esc(detail.artifact)}">${lang==="en"?"Czech NATO categories and chapter outturn (JSON)":"České kategorie NATO a skutečnost kapitoly (JSON)"} ↗</a><a href="${esc(detail.nato_source.url)}" target="_blank" rel="noopener">NATO 2026 ↗</a><a href="${esc(detail.chapter_source.url)}" target="_blank" rel="noopener">MO 2025 ↗</a>`);
