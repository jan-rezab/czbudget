...
export const openapi = {
  ...
  tags: [
    ...
    { name: "Trade", description: "UN Comtrade imports, exports, balances, partners, and products." },
  ...
  paths: {
    ...
    "/api/v1/trade/countries": { get: operation("List trade countries", "Lists reporters with loaded UN Comtrade observations.", ["Trade"]) },
    "/api/v1/trade/rare-earths": { get: operation("Rare earth trade", "Country-reported rare-earth metals and compounds: World totals, separate bilateral partners, weights and dataset coverage. HS 280530, 284610 and 284690; no ores or magnets. Missing data are not zero.", ["Trade"], [{ name: "country", in: "query", required: true, schema: { type: "string", pattern: "^[A-Z]{3}$", example: "CHN" } }, { name: "frequency", in: "query", schema: { type: "string", enum: ["A", "M"], default: "A" } }, { name: "period", in: "query", schema: { type: "string", example: "2024" } }]) },
    "/api/v1/trade": { get: operation("Get trade profile", "Returns import, export, balance, partner and HS chapter aggregates from UN Comtrade.", ["Trade"], [{ name: "country", in: "query", required: true, schema: { type: "string", pattern: "^[A-Z]{3}$", example: "CZE" } }]) },
    "/api/v1/trade/product-partners": { get: operation("Get a product chapter's partners", "Returns the leading import and export partners for one two-digit HS product chapter.", ["Trade"], [{ name: "country", in: "query", required: true, schema: { type: "string", pattern: "^[A-Z]{3}$", example: "CZE" } }, { name: "product", in: "query", required: true, schema: { type: "string", pattern: "^\\d{2}$", example: "87" } }]) },
    "/api/v1/trade/energy/periods": { get: operation("List energy-trade periods", "Lists available annual and monthly periods for crude petroleum, liquefied natural gas, and natural gas in gaseous state, including reporting-market coverage.", ["Trade"]) },
    "/api/v1/trade/energy/flows": { get: operation("Get energy-trade routes", "Returns importer-reported bilateral customs flows for one energy product and period. Routes describe reported origin and importing market, not physical shipping or pipeline paths.", ["Trade"], [
      { name: "product", in: "query", description: "Energy product; defaults to crude petroleum.", schema: { type: "string", enum: ["petroleum", "lng", "gas"], default: "petroleum" } },
      { name: "frequency", in: "query", description: "Annual or monthly observations.", schema: { type: "string", enum: ["A", "M"], default: "A" } },
      { name: "period", in: "query", description: "Year (YYYY) or month (YYYYMM) matching frequency.", schema: { type: "string", pattern: "^\\d{4}(?:0[1-9]|1[0-2])?$", example: "2025" } },
    ]) },
    ...
    "/api/v1/municipalities/CZE/{municipality_id}/cityvizor": { get: operation("Get linked CityVizor publication", "Matches the municipality by exact IČO and returns its voluntary CityVizor profiles, published years, layer counts, and explicitly linked organizations. CityVizor layers overlap the national municipal accounts and are not additive.", ["Cities"], [{ name: "municipality_id", in: "path", required: true, schema: { type: "string", pattern: "^\\d{8}$", example: "00291463" } }]) },
