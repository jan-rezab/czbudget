...
if (municipalTransparency.countries.length !== 44 || municipalTransparency.countries.find((country) => country.iso3 === "BRA")?.pipeline !== "loaded_partial") throw new Error("Expected the 44-country municipal transparency atlas with Brazil's exact partial-load status");
if (globalBudgetTransparency.countries.length !== 195 || globalBudgetTransparency.countries.filter((country) => country.national_budget.research_status === "assessed").length !== 125 || globalBudgetTransparency.countries.filter((country) => country.municipal_item_level.research_status === "researched").length !== 44) throw new Error("Expected a 195-state atlas with 125 national assessments and 44 municipal item-level reviews");
if (globalBudgetTransparency.countries.filter((country) => country.budget_transparency_index?.score !== null).length !== 135 || globalBudgetTransparency.countries.filter((country) => country.psd_coverage?.country_profile === "loaded").length !== 16 || globalBudgetTransparency.countries.filter((country) => country.psd_coverage?.ingestion_status === "discovery_crawl_started").length !== 15) throw new Error("Budget Transparency Index must distinguish indexed, PSD-loaded and crawl-started countries");
...
if (!countryPage.includes('<base href="/">') || !countryPage.includes("country-routes.js") || !countryScript.includes("PSDCountryRoutes.codeFromLocation") || !countryScript.includes("PSDCountryRoutes.href") || !globalNav.includes("/countries/${countrySlugs[code]}") || !countryRoutes.includes('CHE: "switzerland"') || !countryRoutes.includes('BRA: "brazil"') || !countryRoutes.includes('JPN: "japan"') || !countryRoutes.includes('FIN: "finland"')) throw new Error("Country profiles must use readable slug routes throughout the site");
...
if (!nginx.includes("denmark|finland|france") || !nginx.includes("united-states|brazil|spain|japan|netherlands|norway") || nginx.includes("try_files /countries/$1/index.html =404")) throw new Error("All sixteen countries must use the shared national dashboard route");
...
for (const path of await htmlFiles()) {
  ...
  if ((page.match(/global-nav\.js\?v=(?:20260822-component|20260824-identity-outlines|20260824-logo-120|20260824-budget-stages|20260825-country-expansion)/g) || []).length !== 1) throw new Error(`${path}: expected exactly one shared header script`);
