...
if (methodologySources.row_count !== 2117 || methodologySources.countries.length !== 191 || methodologySources.modules.length !== 12 || methodologySources.rows.filter((row) => row.module === "municipal_itemized").length !== 16) throw new Error("Expected the complete global sovereign, municipal and itemized-budget source ledger");
...
if (!nginx.includes("denmark|finland|france") || !nginx.includes("greece|[a-z]{3})/$") || !nginx.includes("greece|[a-z]{3})$") || nginx.includes("try_files /countries/$1/index.html =404")) throw new Error("All IMF-covered countries must use the shared national dashboard route");
