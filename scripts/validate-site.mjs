...
if (countryParity.contract !== "country-parity.v1" || countryParity.countries.length !== 17) throw new Error("Expected the seventeen-country parity contract");
...
if (methodologySources.row_count !== 203 || methodologySources.countries.length !== 17 || methodologySources.modules.length !== 12 || methodologySources.rows.filter((row) => row.module === "municipal_itemized").length !== 16) throw new Error("Expected the complete sovereign, municipal and itemized-budget source ledger");
...
if (!nginx.includes("denmark|finland|france") || !nginx.includes("netherlands|norway|greece)/$") || !nginx.includes("netherlands|norway|greece)$") || nginx.includes("try_files /countries/$1/index.html =404")) throw new Error("All seventeen countries must use the shared national dashboard route");
