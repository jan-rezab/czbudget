...
const processLogPage = await readFile("process/log/index.html", "utf8");
const processLog = JSON.parse(await readFile("data/registry/run-log.v1.json", "utf8"));
const processLogScript = await readFile("run-log.js", "utf8");
...
const defenseComparison = JSON.parse(await readFile("data/defense-comparison.v1.json", "utf8"));
...
const cloudbuildVerify = await readFile("cloudbuild.verify.yaml", "utf8");
const buildPlanes = await readFile("BUILD_PLANES.md", "utf8");
...
if (!methodologyPage.includes('id="release-architecture"') || !methodologyPage.includes('href="/process/log/"')) throw new Error("Methodology must explain the release architecture and link to the operational log");
if (!processLogPage.includes('data-run-log') || processLog.schema_version !== "2.0.0" || !processLog.runs.some((event) => event.event_type === "ingestion") || !processLog.runs.some((event) => event.event_type === "data_release")) throw new Error("Process log must publish ingestion and data-release events");
for (const field of ["git_sha", "pr_number", "cloud_build_id", "image_digest", "data_release_ids"]) if (!processLogScript.includes(field)) throw new Error(`Process log deployment details must expose ${field}`);
...
if (!deepDivePage.includes('href="eu-budget/?code=CZE"') || !globalNav.includes('deep-dives/eu-budget/?code=CZE') || !euBudgetDeepDivePage.includes('id="eu-history-chart"') || !euBudgetDeepDivePage.includes('id="eu-breakdown-bars"') || !euBudgetDeepDivePage.includes('id="eu-comparison-body"') || !euBudgetDeepDivePage.includes('id="state-budget"') || !euBudgetDeepDivePage.includes('id="reconciliation"') || !euBudgetDeepDivePage.includes('id="eu-reference-3"') || !euBudgetDeepDiveScript.includes('eu-budget-flows.v1.json') || euBudgetFlows.contract !== "eu-budget-flows.v1" || euBudgetFlows.countries.length !== 27 || !euBudgetFlows.sources?.page_url || !euBudgetFlows.sources?.annual_accounts_url || !euBudgetFlows.sources?.eca_report_url) throw new Error("EU-budget deep dive must expose the national-budget boundary, history, allocation detail, country reconciliation and source lineage");
...
const euBudget2024 = euBudgetFlows.eu_reconciliation?.find((row) => row.year === 2024);
const euCountrySpending2024 = euBudgetFlows.countries.reduce((sum, country) => sum + country.series.find((row) => row.year === 2024).allocated_spending_m_eur, 0);
const euCountryContributions2024 = euBudgetFlows.countries.reduce((sum, country) => sum + country.series.find((row) => row.year === 2024).national_contribution_m_eur, 0);
const auditedRevenue2024 = Object.values(euBudgetFlows.audited_budget?.revenue_breakdown || {}).reduce((sum, value) => sum + value, 0);
if (!euBudget2024 || Math.abs(euBudget2024.attributed_spending_m_eur - euCountrySpending2024) > 0.01 || Math.abs(euBudget2024.national_contribution_m_eur - euCountryContributions2024) > 0.01 || Math.abs(euBudget2024.accounting_difference_m_eur - (euCountrySpending2024 - euCountryContributions2024)) > 0.01 || auditedRevenue2024 !== euBudgetFlows.audited_budget?.revenue_m_eur || euBudgetFlows.audited_budget?.payments_m_eur !== 246988 || euBudgetFlows.audited_budget?.budget_result_m_eur !== 1345) throw new Error("EU-budget 2024 country comparison and audited revenue account must reconcile exactly");
if (stateEnterpriseCatalogue.records.length < 30 || new Set(stateEnterpriseCatalogue.records.map(record => record.country_code)).size !== 10 || stateEnterpriseCatalogue.records.some(record => (record.source_revenue_m !== null && !Number.isFinite(record.source_revenue_m)) || !stateEnterpriseCatalogue.fx.rates[record.currency] || !record.source_url)) throw new Error("State-enterprise catalogue must contain sourced records and valid available revenues for each tracked country");
...
if (!deepDivePage.includes('href="tax-burden/?code=CZE"') || !deepDivePage.includes('href="redistribution/?code=CZE"') || !globalNav.includes('deep-dives/tax-burden/?code=CZE') || !globalNav.includes('deep-dives/redistribution/?code=CZE') || !taxBurdenPage.includes('data-oecd-chart="tax_wedge"') || !taxBurdenPage.includes('data-oecd-chart="tax_matrix"') || !taxBurdenPage.includes('data-oecd-chart="corporate_rates"') || !taxBurdenPage.includes('data-oecd-chart="carbon_autonomy"') || !redistributionPage.includes('data-oecd-chart="redistribution_hero_stat"') || !redistributionPage.includes('data-oecd-chart="redistribution_summary"') || !redistributionPage.includes('data-oecd-chart="redistribution_bridge"') || !redistributionPage.includes('data-oecd-chart="redistribution_comparison"') || !redistributionPage.includes('data-oecd-chart="socx_composition"') || !redistributionPage.includes('id="sources"')) throw new Error("OECD tax-burden and redistribution reports must expose every promised chart and navigation path");
...
const natoMembers = defenseComparison.countries.filter((country) => country.nato_member);
const defenseMeasures = ["constant_usd", "current_usd", "gdp_share", "govt_share", "per_capita"];
if (!defenseDeepDivePage.includes('id="defense-blocks"') || !defenseDeepDivePage.includes('id="defense-trajectory"') || !defenseDeepDivePage.includes('id="blocks-year"') || !defenseDeepDivePage.includes('id="defense-share"') || !defenseDeepDiveScript.includes("renderUsShare") || !defenseDeepDiveScript.includes("defense-comparison.v1.json") || !defenseDeepDiveScript.includes("BLOCK_VALUE") || natoMembers.length !== 32 || defenseComparison.countries.length !== 38 || defenseComparison.years.at(-1) !== 2025 || defenseComparison.source.sha256 !== "6cc3a30b1064f9f02e60236667eef82e08cad42910ce630909d004ad2c398a9d" || defenseComparison.aggregates.length !== 3 || defenseComparison.aggregates.some((group) => group.series.constant_usd.length !== defenseComparison.years.length) || defenseComparison.countries.some((country) => defenseMeasures.some((measure) => country.series[measure]?.length !== defenseComparison.years.length))) throw new Error("Defense comparison must cover all 32 NATO members plus partners on one pinned SIPRI vintage, with blocks, trajectory, the American-share split and every measure aligned to the year axis");
if (!deepDivePage.includes('href="education/"') || deepDivePage.indexOf('href="education/"') > deepDivePage.indexOf('href="transportation/?code=CZE"') || !globalNav.includes('deep-dives/education/') || !educationDeepDivePage.includes('id="education-country"') || !educationDeepDivePage.includes('id="education-currency"') || !educationDeepDivePage.includes('id="education-flow"') || !educationDeepDivePage.includes('id="education-region-chart"') || !educationDeepDivePage.includes('id="capacity-benchmark-chart"') || !educationDeepDivePage.includes('id="capacity-country-body"') || !educationDeepDivePage.includes('id="coverage-body"') || !cacheBusted(educationDeepDivePage, "education-deep-dive.js") || !educationDeepDiveScript.includes('education-deep-dive.v1.json?v=20260902-global-controls') || !educationDeepDiveScript.includes('education-capacity-international.v1.json?v=20260902-global-controls') || !educationDeepDiveScript.includes('renderCapacityBenchmark') || !educationDeepDiveScript.includes('renderCapacityCountry') || Math.abs(educationDeepDive.headline.consolidated_education_czk_bn - 368.0638) > 0.001 || educationDeepDive.local.regions.length !== 14 || educationDeepDive.capacity.headline_enrolments !== 2226933 || educationDeepDive.capacity.categories.length !== 7 || educationDeepDive.capacity.benchmark.levels.length !== 5 || educationDeepDive.coverage.counts.ready !== 1 || educationDeepDive.coverage.counts.download !== 10 || educationDeepDive.coverage.counts.online !== 6) throw new Error("Education must reconcile Czech finance, capacity, school-type, regional and international benchmark layers");
...
for (const forbidden of ["bq query", "scripts/build_trade_product_intelligence.py", "scripts/merge-municipal-breakdowns.mjs", "publish-public-snapshot", "--publish"]) if (cloudbuild.includes(forbidden)) throw new Error(`Production Cloud Build must remain code-only; found ${forbidden}`);
if (!cloudbuild.includes("read-active-data-release") || !cloudbuild.includes("_STATIC_ASSET_BASE_URL") || !buildPlanes.includes("static-asset releases through published")) throw new Error("Production must read validated data releases through published pointers");
if (!cloudbuildVerify.includes("npx playwright test") || cloudbuildVerify.includes("deploy-immutable.sh") || cloudbuildVerify.includes("publish-public-snapshot") || cloudbuildVerify.includes("bq query")) throw new Error("Full verification must test the release without deploying, publishing or querying the warehouse");
if (!municipalI18n.includes("renderBudgetBreakdown") || !municipalI18n.includes("municipal-budget-codebook.v1.json")) throw new Error("Municipal profiles must surface the detailed FIN 2-12 M breakdown");
...
if (!czechBudgetScript.includes('d=>d.pension],["health","Veřejné zdravotní pojištění",d=>d.health],["care","Příspěvek na péči",d=>d.care]') || /pensionExtra|healthExtra|careExtra/.test(czechBudgetScript)) throw new Error("Annual system costs must use absolute modeled expenditure, not changes from 2025");
...
if (!cloudbuild.includes("_CITYVIZOR_SNAPSHOT_BASE_URL") || !cloudbuild.includes("CITYVIZOR_SNAPSHOT_BASE_URL") || cloudbuild.includes("publish-cityvizor-snapshot")) throw new Error("Code deployment must consume, but never publish, the active CityVizor snapshot");
...
if (!cacheBusted(countryPage, "country-spending.js") || !cacheBusted(countryPage, "country-insights.js") || !cacheBusted(countryPage, "country-dashboard.js") || !cacheBusted(countryPage, "country-dashboard.css") || !countryPage.includes('country-public-entities.js?v=20260823-public-registry-2') || !countryPage.includes('country-public-entities.css?v=20260823-public-registry-2') || !countryPage.includes('country-health.js?v=20260918-chart-integrity') || !countryPage.includes('country-providers.js?v=20260822-network') || !countryPage.includes('country-functions.js?v=20260918-chart-integrity') || !countryPage.includes('country-cash-in.js?v=20260824-loaded-layers') || !countryPage.includes('country-parity.js?v=20260822-parity-contract') || !countryPage.includes('oecd-overlay.js?v=20260828-oecd-key-metrics') || !countryPage.includes('id="data-parity"') || !countryPage.includes('id="cash-in"') || !countryPage.includes('id="oecd-benchmark"') || !countryPage.includes('id="budget-map"') || !countryPage.includes('id="public-entities"') || !countryPage.includes('id="demography"') || !countryPage.includes('id="provider-network"') || !countryPage.includes('id="social-system"') || !countryPage.includes('id="transportation"') || !countryPage.includes('id="recovery"') || !countryScript.includes('function recoveryStory') || !countryScript.includes('countryprofilechange') || !countryInsightsScript.includes("label_en") || !countryPublicEntitiesScript.includes("public-entity-coverage.v1.json") || countryInsightsScript.includes("country-public-entities.v1.json") || !countryDashboardScript.includes('index.id = "country-dashboard-index"') || !countryDashboardScript.includes('ids:["trend", "cash-in", "tax-detail", "oecd-benchmark", "macro", "recovery"]') || !countryDashboardScript.includes('ids:["data-parity", "sources"]') || !countryCashInScript.includes('public-entity-directory/manifest.v1.json')) throw new Error("Fiscal profiles must preserve all shared insight modules and Czech-style dashboard composition");
