...
if (!globalFooter.includes("assets/logo-lockup-dark.svg") || !globalFooter.includes("assets/hlidac-statu-horizontal-inverted-bw.svg") || !globalFooter.includes('<address class="footer-address">${t.address}</address>') || globalFooter.includes('href="#top"') || !globalFooterStyles.includes("background: #171918")) throw new Error("Global footer must use both inverted brand lockups, show only the postal address, and omit the back-to-top link");
...
if (!nginx.includes('location ~ ^(.*/)index\\.html$') || !nginx.includes('return 301 $1$is_args$args;')) throw new Error("Explicit index.html URLs must permanently redirect to their canonical directory URLs");
