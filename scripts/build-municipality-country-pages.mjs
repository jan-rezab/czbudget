...
for(const [slug,page] of selectedPages){
  ...
  const directoryControls=france?`<div class="municipal-controls country-directory-controls france-directory-controls"><label><span data-copy="search">Hledat</span><input id="country-municipality-search" type="search" autocomplete="off" data-placeholder="searchPlaceholder" placeholder="Název, region nebo kód INSEE…"></label><label><span data-copy="year">Rok v profilu</span><select id="country-year-filter"></select></label><label><span data-copy="balanceFilter">Výsledek</span><select id="country-balance-filter"><option value="all" data-copy="allBalances">Přebytky i schodky</option><option value="surplus" data-copy="surplusOnly">Přebytky</option><option value="deficit" data-copy="deficitOnly">Schodky</option></select></label><label><span data-copy="sort">Řazení</span><select id="country-sort"><option value="expenditure" data-copy="sortBudget">Podle velikosti rozpočtu</option><option value="revenue" data-copy="sortRevenue">Podle příjmů</option><option value="balance" data-copy="sortBalance">Podle výsledku</option><option value="population" data-copy="sortPopulation">Podle počtu obyvatel</option><option value="name" data-copy="sortName">Podle názvu</option></select></label><button id="country-reset" type="button" data-copy="reset">Vymazat filtry</button></div><p class="france-directory-order" data-copy="directoryOrderNote">Bez hledání jsou obce seřazené od největšího rozpočtu podle skutečných výdajů.</p>`:`<div class="municipal-controls country-directory-controls"><label><span data-copy="search">Hledat</span><input id="country-municipality-search" type="search" autocomplete="off" data-placeholder="searchPlaceholder" placeholder="Název nebo národní kód…"></label><label><span data-copy="year">Rok</span><select id="country-year-filter"></select></label><button id="country-reset" type="button" data-copy="reset">Vymazat filtry</button></div>`;
...
<head>
  ...
  <script src="../../municipalities-country.js?v=20260902-france-budget-overview" defer></script>
...
<body class="municipalities-page country-municipalities-page" data-country-code="${page.code}" data-country-slug="${slug}">
  ...
  <main id="top">
    ...
    <section id="insights" class="municipal-section"><div class="section-heading"><div><span class="kicker" data-copy="insightsKicker">Datový profil</span><h2 id="insights-title">—</h2></div><p id="insights-copy">—</p></div><div id="country-insight-grid" class="insight-grid"></div></section>
    ${rankingSection}
    ...
    <section id="directory" class="municipal-section"><div class="section-heading"><div><span class="kicker" data-copy="directoryKicker">${france?'Všech 34 875 obcí':'Místní úroveň'}</span><h2 data-copy="directoryTitle">Najděte obec</h2></div><p id="country-directory-count">—</p></div>${directoryControls}<div id="country-municipality-grid" class="municipality-grid"></div><button id="country-load-more" class="load-more" type="button" data-copy="more">Načíst další</button></section>
