...
const COUNTRIES = {
  ...
  GRC:{geo:"EL",name_cs:"Řecko",name_en:"Greece"}
};
...
const NATIONAL_SOURCES = {
  ...
  ESP:{condition_status:"fragmented",condition_year:null,machine_readable:"partial",condition_note_en:"This layer uses harmonised OECD/Eurostat series; no stable current ministry link for the cited infrastructure and budget material was verified.",condition_note_cs:"Tato vrstva používá harmonizované řady OECD/Eurostatu; stabilní aktuální odkaz ministerstva na citované infrastrukturní a rozpočtové podklady nebyl ověřen.",sources:[{title:"OECD ITF · infrastructure investment and maintenance",url:"https://sdmx.oecd.org/public/rest/v1/data/OECD.ITF,DSD_INFRINV@DF_INFRINV,1.0/?startPeriod=2015"}]},
  ...
  GRC:{condition_status:"source_available",condition_year:null,machine_readable:"partial",condition_note_en:"The Ministry of Infrastructure and Transport publishes national plans and project information; the comparable layer uses OECD/Eurostat investment, maintenance and rail series.",condition_note_cs:"Ministerstvo infrastruktury a dopravy zveřejňuje národní plány a projekty; srovnatelná vrstva používá řady OECD/Eurostatu pro investice, údržbu a železnice.",sources:[{title:"Ministry of Infrastructure and Transport Greece",url:"https://www.yme.gr/"},{title:"Hellenic Statistical Authority · transport",url:"https://www.statistics.gr/en/statistics/transport"}]}
};

const PROJECTS = [
  {id:"CZE-D7-KNOVIZ-SLANY",country:"CZE",status_current_verified:false,status_note:"Status retained from the cited historical announcement; not a live construction-status observation.",mode:"road",asset_type:"motorway_widening",project_name:"D7 Knovíz – Slaný-západ",status:"construction",cost_stage:"contract_award",route_km:6.6,cost_local_million:1535.9,currency:"CZK",price_year:2024,tunnel_km:0,urbanity:"mixed",cost_scope:"construction contract; excludes VAT",source:{title:"ŘSD · signed construction contract",url:"https://kraje.rsd.cz/stredocesky/blog/2024/11/27/smlouva-se-zhotovitelem-dostavby-dalnice-d7-mezi-slanym-a-knovizi-uzavrena/"}},
  {id:"CZE-D7-KUTROVICE-PANENSKY-TYNEC",country:"CZE",status_current_verified:false,status_note:"Status retained from the cited historical announcement; not a live construction-status observation.",mode:"road",asset_type:"motorway_widening",project_name:"D7 Kutrovice – Panenský Týnec",status:"construction",cost_stage:"contract_award",route_km:6.764,cost_local_million:1319.404038,currency:"CZK",price_year:2023,tunnel_km:0,urbanity:"rural",cost_scope:"construction contract; excludes VAT",source:{title:"Ministry of Transport · construction start and contract value",url:"https://md.gov.cz/Media/Media-a-tiskove-zpravy/Reditelstvi-silnic-a-dalnic-zacalo-stavet-novy-use?returl=%2FMedia%2FMedia-a-tiskove-zpravy%2FZ-Planetaria-az-ke-hvezdam-Cesko-rekapituluje-svo"}},
  {id:"CZE-D0-518-RUZYNE-SUCHDOL",country:"CZE",status_current_verified:false,status_note:"Status retained from the cited historical announcement; not a live construction-status observation.",mode:"road",asset_type:"motorway_tunnel_heavy",project_name:"D0 518 Ruzyně – Suchdol",status:"planned",cost_stage:"official_estimate",route_km:8.26,cost_local_million:10658.98,currency:"CZK",price_year:2024,tunnel_km:3.45,urbanity:"urban",cost_scope:"official construction-cost estimate; excludes VAT",source:{title:"ŘSD · northern Prague ring preparation",url:"https://kraje.rsd.cz/stredocesky/blog/2024/09/03/priprava-dostavby-severni-casti-prazskeho-okruhu-pokrocila/"}},
...
// Outlier detection: a single-year jump larger than this factor (up or down) versus the
// prior year is almost always incomplete/late reporting rather than a real change, so the
// LATER observation is flagged rather than dropped. Threshold chosen so ordinary swings in
// investment/maintenance spending (which can legitimately double or halve year over year)
// are left alone, while a >4x move — the kind produced by a not-yet-fully-reported latest
// year — is surfaced. Mirrors the `quality_flags`/`status` convention already used in
// data/road-network-history.v1.json for suspected series breaks or reclassifications.
const OUTLIER_FACTOR=4;
function flagOutliers(series) {
  for(let index=1;index<series.length;index++) {
    const previous=series[index-1].value, current=series[index].value;
    if(!Number.isFinite(previous)||!Number.isFinite(current)||previous===0)continue;
    const ratio=current/previous;
    if(ratio>=OUTLIER_FACTOR||ratio<=1/OUTLIER_FACTOR) {
      series[index]={...series[index],status:series[index].status??"b",quality_flags:[...(series[index].quality_flags??[]),"possible_series_break_or_reclassification"]};
    }
  }
  return series;
}
...
const countries=Object.fromEntries(Object.entries(COUNTRIES).map(([code,country])=>[code,{...country,rail:railByCountry[code],infrastructure_spending:{road:{investment_constant_eur:flagOutliers(oecdSeries(code,"INV","ROAD","Q")),investment_current_eur:flagOutliers(oecdSeries(code,"INV","ROAD","V")),maintenance_constant_eur:flagOutliers(oecdSeries(code,"MNT","ROAD","Q")),maintenance_current_eur:flagOutliers(oecdSeries(code,"MNT","ROAD","V"))},motorways:{investment_constant_eur:flagOutliers(oecdSeries(code,"INV","MOTORWAYS","Q")),maintenance_constant_eur:flagOutliers(oecdSeries(code,"MNT","MOTORWAYS","Q"))},rail:{investment_constant_eur:flagOutliers(oecdSeries(code,"INV","RAIL","Q")),investment_current_eur:flagOutliers(oecdSeries(code,"INV","RAIL","V")),maintenance_constant_eur:flagOutliers(oecdSeries(code,"MNT","RAIL","Q")),maintenance_current_eur:flagOutliers(oecdSeries(code,"MNT","RAIL","V"))}},condition_and_repairs:NATIONAL_SOURCES[code]}]));

const payload={schema_version:"1.0.0",generated_at:new Date().toISOString(),methodology:{en:"Infrastructure investment and maintenance are kept separate and use OECD-ITF definitions. Constant-euro series use the OECD 2015 price base. Rail network and traffic use Eurostat definitions. Condition measures retain national definitions and are never ranked without a shared denominator. Project cost per kilometre is calculated only for records with an explicit cost stage, scope and route length; route-km and lane-km must not be mixed. Any observation more than 4x above or below the prior year is marked with a quality flag as a suspected incomplete or not-yet-finalised latest-year report rather than a real change.",cs:"Investice do infrastruktury a údržba zůstávají oddělené a používají definice OECD-ITF. Řady ve stálých eurech používají cenovou bázi OECD 2015. Železniční síť a provoz používají definice Eurostatu. Ukazatele stavu zachovávají národní definice a bez společného jmenovatele se neřadí. Náklady na kilometr se počítají jen u záznamů s výslovnou fází nákladu, rozsahem a délkou trasy; traťové a pruhové kilometry se nesmějí míchat. Pozorování, které je více než 4× nad nebo pod hodnotou předchozího roku, je označeno příznakem kvality jako podezření na neúplné nebo dosud neuzavřené vykázání posledního roku, nikoli jako skutečná změna."},countries,projects:PROJECTS,cost_method:{required_fields:["cost_stage","cost_scope","route_km","currency","price_year","source"],comparison_groups:["greenfield_motorway","motorway_widening","expressway","ordinary_bypass","surface_rehabilitation","bridge_or_tunnel_heavy","conventional_rail","rail_electrification","high_speed_rail","signalling_or_station"],display:["local_currency_per_route_km","constant_eur_per_route_km","cost_per_lane_km_when_lanes_are_known","planned_vs_awarded_vs_final_outturn","median_and_interquartile_range"]},sources:[{id:"oecd-itf-infrinv",title:"OECD ITF · Transport infrastructure investment and maintenance spending",url:"https://data-explorer.oecd.org/vis?df%5Bag%5D=OECD.ITF&df%5Bds%5D=dsDisseminateFinalDMZ&df%5Bid%5D=DSD_INFRINV%40DF_INFRINV&df%5Bvs%5D=1.0",api_url:OECD_URL},{id:"eurostat-rail",title:"Eurostat · railway transport database",url:"https://ec.europa.eu/eurostat/en/web/transport/database"},{id:"eu-rmms",title:"European Commission · Rail Market Monitoring",url:"https://transport.ec.europa.eu/transport-modes/rail/market/rail-market-monitoring-rmms_en"}]};
