...
if (!dataOnly) {
  ...
  const industryPages = ["deep-dives/industry/index.html", "deep-dives/industry/diagnostics/index.html"].filter(p => existsSync(path.join(root, p))).length;
  const czechSourcePages = existsSync(path.join(root, "czech-sources.html")) ? 1 : 0;
  const cityvizorPages = existsSync(path.join(root, "cityvizor/index.html")) ? 1 : 0;
  const moneyFlowPages = existsSync(path.join(root, "money-flow.html")) ? 1 : 0;
  const processLogPages = existsSync(path.join(root, "process/log/index.html")) ? 1 : 0;
  const expectedSitemapUrls = municipalities.length + 1 + municipalityCountryPaths.length + 15 + 6 + 6 + benchmarkMunicipalities.length + 4 + countryPaths.length + expansionProfiles.length + 7 + municipalSpecialPages + productMarketPages + accountabilityPages + industryPages + czechSourcePages + cityvizorPages + moneyFlowPages + processLogPages;
  ...
  for (const publicPath of ["/", "/money-flow.html", "/process/log/", "/czech-sources.html", "/cityvizor/", "/cesko.html", "/cesky-rozpocet.html", "/eu-capitals.html", ...countryPaths, "/municipalities/", ...municipalityCountryPaths, "/deep-dives/", "/deep-dives/eu-budget/", "/deep-dives/education/", "/deep-dives/transportation/", "/deep-dives/health/", "/deep-dives/state-owned-enterprises/", "/deep-dives/capital-cities/", "/deep-dives/revenue/", "/deep-dives/ageing/", "/deep-dives/migration/", "/deep-dives/defense/", "/deep-dives/tax-burden/", "/deep-dives/redistribution/", "/deep-dives/trade/", "/deep-dives/product-markets/", "/deep-dives/industry/", "/deep-dives/industry/diagnostics/", "/deep-dives/budget-planner/", "/deep-dives/plzen-contracts/", "/deep-dives/digital-spillover/", "/deep-dives/public-employment/", "/cz/municipalities/", "/cz/mesta/", "/cz/kraje/", "/cz/kraje/accountability/"]) {
