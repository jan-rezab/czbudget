...
(function (root) {
  ...
  function render(host, spec) {
    ...
    const width = 1120, height = spec.height || (spec.type === 'bar' ? Math.max(260, rows.length * 48 + 90) : 430);
    ...
    if (spec.type === 'bar') {
      ...
      marks = rows.map((row, i) => `<text x="${left - 12}" y="${top + i * rowHeight + 22}" text-anchor="end">${escape(row.label.length > 23 ? row.label.slice(0, 22) + '…' : row.label)}</text>` + fields.map((field, f) => row.values[f] === null ? '' : `<rect x="${Math.min(bx(0), bx(row.values[f]))}" y="${top + i * rowHeight + f * 28 / fields.length}" width="${Math.abs(bx(row.values[f]) - bx(0))}" height="${26 / fields.length}" fill="${escape(row.values[f] < 0 ? '#c93237' : field.color)}"/>`).join('')).join('');
