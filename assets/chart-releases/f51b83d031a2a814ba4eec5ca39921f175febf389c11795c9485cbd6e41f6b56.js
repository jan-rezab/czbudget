...
(function (root) {
  ...
  function render(host, spec) {
    ...
    on(host, 'click', e => { const hit = e.target.closest('[data-point]'); if (hit) { pinned = !pinned; if (pinned) show(hit, e.clientX); else hide(); if (typeof spec.onSelect === 'function') spec.onSelect(spec.rows[Number(hit.dataset.point)]); } });
    ...
    return { data, destroy: host.__psdChartCleanup };
  }
