...
(function (root) {
  ...
  function model(spec) {
    ...
    return { fields, rows, axis: spec.type === 'stacked' ? { min: 0, max: 100, ticks: [0, 25, 50, 75, 100] } : domain(rows.flatMap(row => row.values)) };
  }
  function render(host, spec) {
    ...
    const width = 1120, height = spec.type === 'bar' ? Math.max(260, rows.length * 48 + 90) : 430;
    const left = spec.type === 'bar' ? 200 : 82, right = 30, top = 38, bottom = 52;
    ...
    const years = rows.map((row, i) => i % Math.max(1, Math.ceil(rows.length / 10)) === 0 || i === rows.length - 1 ? `<text x="${x(i)}" y="${height - 20}" text-anchor="middle">${escape(row.label)}</text>` : '').join('');
    let marks = '';
    ...
    if (spec.type === 'bar') {
      ...
      marks = rows.map((row, i) => `<text x="${left - 12}" y="${top + i * 48 + 22}" text-anchor="end">${escape(row.label)}</text>` + fields.map((field, f) => row.values[f] === null ? '' : `<rect x="${Math.min(bx(0), bx(row.values[f]))}" y="${top + i * 48 + f * 28 / fields.length}" width="${Math.abs(bx(row.values[f]) - bx(0))}" height="${26 / fields.length}" fill="${escape(field.color)}"/>`).join('')).join('');
      axes = `<g class="psd-plot-grid"><line x1="${bx(0)}" x2="${bx(0)}" y1="${top}" y2="${height - bottom}"/><text x="${left}" y="20">${escape(spec.unit || '')}</text></g>`;
      hits = rows.map((row, i) => `<rect class="psd-plot-hit" data-point="${i}" x="0" y="${top + i * 48}" width="${width}" height="44" tabindex="${i ? -1 : 0}" role="button" aria-label="${escape(describe(row))}"/>`).join('');
    }
    const empty = !rows.some(row => row.values.some(finite));
    host.innerHTML = `${empty ? `<p class="psd-chart-empty">${escape(spec.emptyLabel || 'No reported values')}</p>` : ''}<svg viewBox="0 0 ${width} ${height}" role="group" aria-label="${escape(spec.title || fields.map(f => f.label).join(', '))}">${axes}${marks}<line class="psd-plot-guide" y1="${top}" y2="${height - bottom}" hidden/>${hits}</svg><div class="psd-plot-tooltip" role="status" aria-live="polite" hidden></div>`;
    ...
    on(host, 'click', e => { const hit = e.target.closest('[data-point]'); if (hit) { pinned = !pinned; if (pinned) show(hit, e.clientX); else hide(); } });
    ...
    on(host, 'keydown', e => {
      ...
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); current.click(); return; }
    ...
    return { data, destroy: () => abort.abort() };
  }
  const api = Object.freeze({ render, model, domain, palette });
