...
if (historyRoot) {
  ...
  const compact = (value) => Number.isFinite(value) ? `${value < 0 ? "−" : ""}${fmt.format(Math.abs(value) / 1e9)} mld. Kč` : "—";
  ...
  function render(city) {
    ...
    kpis.innerHTML = `<article><span>${english ? "Revenue" : "Příjmy"} ${latest.year}</span><strong>${compact(latest.revenue_actual)}</strong></article><article><span>${english ? "Balance" : "Výsledek"} ${latest.year}</span><strong class="${latest.budget_balance >= 0 ? "positive" : "negative"}">${compact(latest.budget_balance)}</strong></article><article><span>${english ? "Cash and deposits" : "Stav účtů"} ${latest.year}</span><strong>${compact(latest.cash_current)}</strong></article><article><span>${english ? `${yearCount}-year cumulative balance` : `Součet výsledků za ${yearCount} let`}</span><strong class="${cumulative >= 0 ? "positive" : "negative"}">${compact(cumulative)}</strong><small>${english ? `${surplusYears} of ${yearCount} years in surplus` : `${surplusYears} z ${yearCount} let v přebytku`}</small></article>`;
    const width = 1120, height = 460, left = 72, right = 26, top = 30, bottom = 54;
    const values = series.flatMap((row) => [row.revenue_actual, row.expense_actual, row.cash_current]).filter(Number.isFinite);
    ...
    const points = series.map((row, index) => Number.isFinite(row.cash_current) ? `<g class="history-point"><circle cx="${x(index)}" cy="${y(row.cash_current)}" r="4"><title>${row.year}: ${compact(row.cash_current)}</title></circle></g>` : "").join("");
