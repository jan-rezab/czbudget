...
const state = { code:P.get("code")||"CZE", lang:P.get("lang")||localStorage.getItem("psd-lang")||"cs", year:2024, chart:"balance_pct_gdp", currency:"local", data:null, catalog:null, fx:null, ministries:null };
const $ = selector => document.querySelector(selector);
const T = {
  cs:{back:"← Všechny země",navTrend:"Vývoj",navMacro:"Makro",navSpecifics:"Specifika",navSources:"Zdroje",profileEyebrow:"Detail země · veřejné finance",switchCountry:"Změnit zemi",switchYear:"Rok profilu",currencyView:"Měna",localCurrency:"Místní",fiscalSnapshot:"Fiskální snapshot",trendKicker:"01 / Dvacetiletý vývoj",trendTitle:"Rozpočet v čase.",trendCopy:"Harmonizované ukazatele IMF umožňují číst dlouhodobý fiskální příběh ve společné definici.",balance:"Saldo",debt:"Dluh",expense:"Výdaje",revenue:"Příjmy",gdpPcTitle:"HDP na obyvatele",macroKicker:"02 / Makro kontext",macroTitle:"Výkon, dluh a kupní síla.",macroCopy:"Měnové částky se na detailu zobrazují v místní měně; přepínač je přepočítá na EUR. PPP zůstává v mezinárodních dolarech.",debtRatioTitle:"Hrubý dluh k HDP",pppTitle:"HDP na obyvatele v paritě kupní síly",specificKicker:"03 / Národní specifika",specificTitle:"Jak země hlídá rozpočet.",specificCopy:"Vedle výsledku ukazujeme institucionální pravidla a četnost přebytku, ne jen jeden vybraný rok.",sourcesKicker:"04 / Primární zdroje",sourcesTitle:"Od srovnání k rozpočtovým kapitolám.",sourcesCopy:"Ukládáme originální soubory a jejich kontrolní součty; mapování ministerstev bude následovat nad touto reprodukovatelnou vrstvou.",footerSource:"Srovnávací řady: IMF WEO · kurzy: ECB · národní zdroje jsou uvedené výše",backTop:"Nahoru ↑",primary:"Primární saldo",growth:"Růst HDP",inflation:"Inflace",unemployment:"Nezaměstnanost",actual:"skutečnost",estimate:"odhad",governmentScope:"Rozsah národního rozpočtu",fiscalRule:"Fiskální pravidlo",surplusRecord:"Roky s přebytkem",openSource:"Otevřít originál ↗",ministryData:"Detail ministerstev",downloaded:"staženo a ověřeno",mappingReady:"připraveno k mapování",noRule:"Pro tuto zemi zatím zobrazujeme empirický výsledek; právní rámec doplníme z primárního zdroje.",sourcePurpose:"Primární národní zdroj rozpočtu a jeho plnění.",czechLocalTitle:"Česká územní vrstva",czechLocalCta:"Otevřít obce a kraje →"},
  en:{back:"← All countries",navTrend:"Trend",navMacro:"Macro",navSpecifics:"Specifics",navSources:"Sources",profileEyebrow:"Country detail · public finance",switchCountry:"Change country",switchYear:"Profile year",currencyView:"Currency",localCurrency:"Local",fiscalSnapshot:"Fiscal snapshot",trendKicker:"01 / Twenty-year trend",trendTitle:"The budget over time.",trendCopy:"Harmonised IMF indicators reveal the long-run fiscal story under a common definition.",balance:"Balance",debt:"Debt",expense:"Expenditure",revenue:"Revenue",gdpPcTitle:"GDP per capita",macroKicker:"02 / Macro context",macroTitle:"Output, debt and purchasing power.",macroCopy:"Country details show monetary amounts in local currency by default; the toggle converts them to EUR. PPP stays in international dollars.",debtRatioTitle:"Gross debt to GDP",pppTitle:"GDP per capita at purchasing-power parity",specificKicker:"03 / National specifics",specificTitle:"How the country disciplines its budget.",specificCopy:"Alongside the latest result we show institutional rules and the frequency of surpluses across the full period.",sourcesKicker:"04 / Primary sources",sourcesTitle:"From comparison to budget departments.",sourcesCopy:"We preserve original files and checksums; ministry mapping will build on this reproducible source layer.",footerSource:"Comparable series: IMF WEO · exchange rates: ECB · national sources above",backTop:"Back to top ↑",primary:"Primary balance",growth:"GDP growth",inflation:"Inflation",unemployment:"Unemployment",actual:"actual",estimate:"estimate",governmentScope:"National budget scope",fiscalRule:"Fiscal rule",surplusRecord:"Years in surplus",openSource:"Open original ↗",ministryData:"Ministry detail",downloaded:"downloaded and verified",mappingReady:"ready for mapping",noRule:"For this country the page currently shows the empirical record; the legal framework will be added from a primary source.",sourcePurpose:"Primary national source for the budget and its execution.",czechLocalTitle:"Czech territorial layer",czechLocalCta:"Open cities and regions →"}
};
...
function header(){
  const c=meta(); $("#country-code").textContent=c.country_code; $("#country-name").textContent=name(); $("#footer-country").textContent=name();
  $("#country-subtitle").textContent=`${c.currency_code} · IMF WEO 2005–2024 · ${state.currency==="local"?(state.lang==="en"?"local-currency view":"zobrazení v místní měně"):"EUR"}`;
...
function charts(){
  ...
  $("#fiscal-chart-title").textContent=`${fiscalLabels[state.chart]} / % HDP`;
  lineChart("country-chart",series().metrics[state.chart].values,{format:v=>fmt(v,"%",false,0),zero:state.chart==="balance_pct_gdp",color:state.chart==="balance_pct_gdp"?"#b13a33":"#0b5d4a"});
  ...
  $("#gdp-unit-label").textContent=moneyCode(); lineChart("gdp-per-capita-chart",gdp,{format:v=>money(v),color:"#0b5d4a",source:"IMF WEO · ECB"});
  lineChart("debt-ratio-chart",series().metrics.gross_debt_pct_gdp.values,{format:v=>fmt(v,"%",false,0),color:"#b13a33"});
  lineChart("ppp-chart",series().metrics.gdp_per_capita_ppp.values,{format:v=>`${Math.round(v/1000)}k`,color:"#9a7d20"});
}
function specifics(){
...
function sources(){
  ...
  $("#czech-local-link").innerHTML=state.code==="CZE"?`<a class="czech-local-card" href="cz/obce/"><div><span>CZ / LOCAL</span><h3>${T[state.lang].czechLocalTitle}</h3></div><b>${T[state.lang].czechLocalCta}</b></a>`:"";
}
function render(){translate();header();snapshot();charts();specifics();sources()}
function init(){
  ...
  document.querySelectorAll("[data-chart]").forEach(b=>b.onclick=()=>{state.chart=b.dataset.chart;document.querySelectorAll("[data-chart]").forEach(x=>x.classList.toggle("active",x===b));charts()});
  render();
}
Promise.all([
