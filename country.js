...
const state = { code:window.PSDCountryRoutes.codeFromLocation(), lang:P.get("lang")||localStorage.getItem("psd-lang")||"cs", year:2024, chartView:"ratio", currency:"local", data:null, catalog:null, fx:null, ministries:null };
...
function init(){
  ...
  document.querySelectorAll("[data-lang]").forEach(b=>b.onclick=()=>{state.lang=b.dataset.lang;localStorage.setItem("psd-lang",state.lang);history.replaceState(null,"",window.PSDCountryRoutes.href(state.code,state.lang,location.hash));render()});
