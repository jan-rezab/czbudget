(() => {
  ...
  function render() {
    ...
    root.innerHTML = `<div class="category-compare-heading"><div><span class="kicker">${t.kicker}</span><h2>${t.title}</h2></div><p>${t.intro}</p></div>
      ...
      <div class="category-summary"><article><span>${t.leader}</span><strong>${leader.country.code} · ${pct(leader.share)}</strong><small>${countryName(leader.country.code)}</small></article><article><span>${t.median}</span><strong>${pct(median)}</strong><small>${categoryLabel(selectedCategory)}</small></article><article><span>${t.coverage}</span><strong>10 / 10</strong><small>${t.countries}</small></article></div>
      ...
      <div class="category-matrix-wrap"><table class="category-matrix"><thead><tr><th>${t.category}</th>${state.data.countries.map(country => `<th><img src="assets/flags/${flags[country.code]}.svg" alt="">${country.code}</th>`).join("")}</tr></thead><tbody>${state.data.categories.map(item => `<tr class="${item.id === state.category ? "selected" : ""}" data-category-row="${item.id}"><th><button type="button" data-category="${item.id}">${esc(categoryLabel(item))}</button></th>${state.data.countries.map(country => { const value=country.groups.find(group=>group.category_id===item.id).shares_pct[state.period]; const alpha=Math.min(.78,.08+Math.max(0,value)/55); return `<td style="--heat:${alpha}">${esc(pct(value))}</td>`; }).join("")}</tr>`).join("")}</tbody></table></div>
