(() => {
  ...
  const state = {lang:window.PSDLanguage?.current() || (document.documentElement.lang === "en" ? "en" : "cs"),category:"social_protection",period:"current",measure:"share",compare:"CZE",data:null};
  ...
  function render() {
    ...
    const countryOptions=state.data.countries.slice().sort((a,b)=>countryName(a.code).localeCompare(countryName(b.code),locale())).map(country=>`<option value="${country.code}" ${country.code===state.compare?"selected":""}>${esc(countryName(country.code))}</option>`).join("");
    ...
    root.querySelector("#category-country").addEventListener("change",event=>{state.compare=event.target.value;render();});
