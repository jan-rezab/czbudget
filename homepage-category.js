(() => {
  ...
  function render() {
    ...
    root.innerHTML = `<div class="category-compare-heading"><div><span class="kicker">${t.kicker}</span><h2>${t.title}</h2></div><p>${t.intro}</p></div>
      ...
      <div class="category-summary"><article><span>${t.leader}</span><strong>${leader.country.code} · ${pct(leader.share)}</strong><a href="country.html?code=${leader.country.code}&lang=${state.lang}#spending">${countryName(leader.country.code)} ↗</a></article><article><span>${t.median}</span><strong>${pct(median)}</strong><small>${categoryLabel(selectedCategory)}</small></article><article><span>${t.coverage}</span><strong>10 / 10</strong><small>${t.countries}</small></article></div>
