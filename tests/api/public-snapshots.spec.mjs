...
const czechProfile = { entity: { national_id: "00262978", short_name: "Liberec" } };
const czechHistory = { schema_version: "1.0.0", period: { from: 2010, to: 2025 }, series: [{ year: 2025, revenue_actual: 20 }] };
const payloadHash = crypto.createHash("sha256").update(`${JSON.stringify(profile)}\n${JSON.stringify(history)}`).digest("hex");
const czechPayloadHash = crypto.createHash("sha256").update(`${JSON.stringify(czechProfile)}\n${JSON.stringify(czechHistory)}`).digest("hex");
...
const czechRoute = { path: "/cz/municipalities/liberec/", profile_id: "CZE:00262978", country_code: "CZE", entity_code: "00262978", entity_name: "Liberec", shard_key: objectKey, payload_sha256: czechPayloadHash };
...
const shardBody = `${JSON.stringify({
  ...
  schema_version: "1.0.0", release_id: releaseId, profile_id: czechRoute.profile_id,
  canonical_path: czechRoute.path, payload_sha256: czechPayloadHash, profile: czechProfile, history: czechHistory,
})}\n`;
...
await fs.writeFile(path.join(fixtureRoot, `releases/${releaseId}/routes.v1.json.gz`), gzipSync(JSON.stringify({
  schema_version: "2.0.0", release_id: releaseId, routes: [route, czechRoute],
  shards: { "000": { object_key: objectKey, profile_count: 2, body_sha256: crypto.createHash("sha256").update(shardBody).digest("hex") } },
})));
...
test("snapshot store resolves and verifies an immutable profile", async () => {
  ...
  assert.equal((await store.status()).route_count, 2);
});
...
test("published Czech history JSON path resolves by IČO through the snapshot release", async () => {
  const server = http.createServer(handler);
  await new Promise((resolve) => server.listen(0, "127.0.0.1", resolve));
  const url = `http://127.0.0.1:${server.address().port}/data/municipal-history/00262978.json`;
  try {
    const response = await fetch(url);
    assert.equal(response.status, 200);
    assert.deepEqual(await response.json(), czechHistory);
