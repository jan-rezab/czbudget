...
(function (root) {
  ...
  const defaults = { metric: 'expenditure_pct_gdp', countries: ['CZE', 'DEU', 'GBR', 'USA'], mode: 'level', view: 'line', scale: 'fit' };
  const metricsFor = dataset => dataset.kind === 'trade' ? dataset.metrics : metrics;
  ...
  function normalize(input, dataset) {
    ...
    const availableMetrics = metricsFor(dataset);
    return { metric: Object.hasOwn(availableMetrics, input.metric) ? input.metric : Object.keys(availableMetrics)[0], countries: countries.length ? countries : dataset.countries.slice(0, 1).map(c => c.country_code), start, end, year: Math.max(start, Math.min(end, clamp(input.year ?? end))), mode: input.mode === 'change' ? 'change' : 'level', view: ['line', 'bar', 'table'].includes(input.view) ? input.view : 'line', scale: input.scale === 'zero' ? 'zero' : 'fit' };
  }
  function observation(dataset, code, metric, year) {
    ...
    if (dataset.kind === 'trade') return point?.status === 'loaded' && finite(point.value) ? point.value : null;
  ...
  function build(dataset, input) {
    ...
    const fields = state.countries.map((code, index) => ({ key: code, label: dataset.countries.find(c => c.country_code === code).name_en, color: ['#a8b63f', '#171918', '#8b8d83', '#171918'][index], dash: ['', '', '', '8 5'][index] }));
    const contextRows = Array.from({ length: dataset.period.end_year - dataset.period.start_year + 1 }, (_, i) => {
      const year = dataset.period.start_year + i;
    ...
    const rows = contextRows.filter(row => row.year >= state.start && row.year <= state.end);
    ...
    const unit = dataset.kind === 'trade' ? (state.mode === 'change' ? 'Change in current USD' : 'Current USD') : (state.mode === 'change' ? 'Percentage-point change' : '% of GDP');
    return { state, fields, rows, contextRows, snapshot, unit };
  }
  const api = Object.freeze({ metrics, metricsFor, defaults, normalize, observation, build });
