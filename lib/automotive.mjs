...
export function monthlySeries(data, { market = 'ALL', metric = 'value', scope = 'external', start, end } = {}) {
  ...
  return SEGMENTS.map(segment => {
    const points = periods.map(period => {
      ...
      const field = scope === 'all' ? 'values_all' : 'values';
      const values = Object.fromEntries(REGIONS.map(region => [region, complete ? relevant.reduce((sum, r) => sum + r[field][region], 0) : null]));
...
export function tradeRoutes(data, { segment = 'vehicles', period, market = 'ALL', geography = 'regions', origin = 'ALL', scope = 'external' } = {}) {
  const origins = new Map((data.origins || []).map(o => [o.code, o]));
  const eu27 = new Set(data.eu27 || []);
  const totals = new Map();
  for (const row of data.routes || []) {
    if (row.segment !== segment || row.period !== period || (market !== 'ALL' && row.market !== market)) continue;
    if (scope !== 'all' && origins.get(row.origin)?.region === 'EU27' && eu27.has(row.market)) continue;
