(() => {
  ...
  const copy = {
    cs:{home:"Domů",compare:"Srovnání",country:"Země",capitals:"Evropská hlavní města",method:"Metodika",all:"Všechny profily"},
    en:{home:"Home",compare:"Compare",country:"Country",capitals:"European capitals",method:"Methodology",all:"All profiles"}
  };
  function render() {
    ...
    document.querySelectorAll(".global-nav").forEach(nav => {
      ...
      nav.innerHTML = `<a href="index.html?lang=${lang}" data-global-nav="home">${t.home}</a><a href="index.html?lang=${lang}#compare" data-global-nav="compare">${t.compare}</a><details class="country-menu"><summary>${t.country}<span aria-hidden="true">⌄</span></summary><div class="country-menu-panel"><div class="country-menu-head"><span>${t.country}</span><a href="index.html?lang=${lang}#countries">${t.all} →</a></div><a class="capital-menu-feature" href="eu-capitals.html?lang=${lang}"><b>EU27+</b><span>${t.capitals}</span></a>${countries.map(([code,cs,en])=>`<a href="${countryHref(code)}"><b>${code}</b><span>${lang === "en" ? en : cs}</span></a>`).join("")}</div></details><a href="index.html?lang=${lang}#method" data-global-nav="method">${t.method}</a>`;
      const active = page === "country.html" || page === "cesky-rozpocet.html" || page === "eu-capitals.html" ? "country" : location.hash === "#compare" ? "compare" : location.hash === "#method" ? "method" : page === "index.html" || page === "" ? "home" : "";
