(() => {
  ...
  class PsdSiteHeader extends HTMLElement {
    ...
    renderNavigation() {
      ...
      nav.querySelector(".deep-dive-menu-panel")?.insertAdjacentHTML("beforeend", `<a href="${assetRoot}deep-dives/economy/?code=CZE&lang=${lang}"><b>09</b><span><strong>${t.economy}</strong><small>${t.economyCopy}</small></span></a><a href="${assetRoot}deep-dives/defense/?code=USA&lang=${lang}"><b>10</b><span><strong>${t.defense}</strong><small>${t.defenseCopy}</small></span></a><a href="${assetRoot}deep-dives/tax-burden/?code=CZE&lang=${lang}"><b>11</b><span><strong>${t.taxBurden}</strong><small>${t.taxBurdenCopy}</small></span></a><a href="${assetRoot}deep-dives/redistribution/?code=CZE&lang=${lang}"><b>12</b><span><strong>${t.redistribution}</strong><small>${t.redistributionCopy}</small></span></a><a href="${assetRoot}deep-dives/money/cze/?lang=${lang}"><b>13</b><span><strong>${t.money}</strong><small>${t.moneyCopy}</small></span></a><a href="${assetRoot}deep-dives/budget-planner/?lang=${lang}"><b>15</b><span><strong>${t.budgetPlanner}</strong><small>${t.budgetPlannerCopy}</small></span></a>`);
