(() => {
  ...
  function render() {
    ...
    document.querySelectorAll(".global-nav").forEach(nav => {
      const countryHref = code => `country.html?code=${code}&lang=${lang}`;
      const capitalsLink = `<a href="eu-capitals.html?lang=${lang}" data-global-nav="capitals">${t.capitals}</a>`;
      nav.innerHTML = `<a href="index.html?lang=${lang}" data-global-nav="home">${t.home}</a><a href="index.html?lang=${lang}#compare" data-global-nav="compare">${t.compare}</a><a href="cz/obce/?lang=${lang}" data-global-nav="cities">${t.cities}</a>${capitalsLink}<details class="country-menu"><summary>${t.country}<span aria-hidden="true">⌄</span></summary><div class="country-menu-panel"><div class="country-menu-head"><span>${t.country}</span><a href="index.html?lang=${lang}#countries">${t.all} →</a></div><a class="capital-menu-feature" href="cesky-rozpocet.html?lang=${lang}"><b>CZ+</b><span>${t.czechBudget}</span></a>${countries.map(([code,cs,en])=>`<a href="${countryHref(code)}"><img src="${assetRoot}assets/flags/${flags[code]}.svg" alt=""><b>${code}</b><span>${lang === "en" ? en : cs}</span></a>`).join("")}</div></details><a href="index.html?lang=${lang}#method" data-global-nav="method">${t.method}</a>`;
