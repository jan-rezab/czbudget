(() => {
  ...
  const copy = {
    cs:{home:"Domů",compare:"Srovnání",cities:"Obce",country:"Země",capitals:"EU města",method:"Metodika",all:"Všechny profily",czechBudget:"Český státní rozpočet"},
    en:{home:"Home",compare:"Compare",cities:"Municipalities",country:"Country",capitals:"EU cities",method:"Methodology",all:"All profiles",czechBudget:"Czech state budget"}
  };
  function render() {
    ...
    document.querySelectorAll(".global-nav").forEach(nav => {
      ...
      nav.innerHTML = `<a href="${assetRoot}index.html?lang=${lang}" data-global-nav="home">${t.home}</a><a href="${assetRoot}index.html?lang=${lang}#compare" data-global-nav="compare">${t.compare}</a><a href="${assetRoot}municipalities.html?lang=${lang}" data-global-nav="cities">${t.cities}</a>${capitalsLink}<details class="country-menu"><summary>${t.country}<span aria-hidden="true">⌄</span></summary><div class="country-menu-panel"><div class="country-menu-head"><span>${t.country}</span><a href="${assetRoot}index.html?lang=${lang}#countries">${t.all} →</a></div><a class="capital-menu-feature" href="${assetRoot}cesky-rozpocet.html?lang=${lang}"><b>CZ+</b><span>${t.czechBudget}</span></a>${countries.map(([code,cs,en])=>`<a href="${countryHref(code)}"><img src="${assetRoot}assets/flags/${flags[code]}.svg" alt=""><b>${code}</b><span>${lang === "en" ? en : cs}</span></a>`).join("")}</div></details><a href="${assetRoot}index.html?lang=${lang}#method" data-global-nav="method">${t.method}</a>`;
      const active = page === "municipalities.html" || location.pathname.includes("/cz/mesta/") || location.pathname.includes("/cz/obce/") || location.pathname.includes("/cz/kraje/") ? "cities" : page === "eu-capitals.html" ? "capitals" : page === "country.html" || page === "cesky-rozpocet.html" ? "country" : location.hash === "#compare" ? "compare" : location.hash === "#method" ? "method" : page === "index.html" || page === "" ? "home" : "";
