(() => {
  ...
  function enhanceSelect(select) {
    ...
    const list = document.createElement("span"); list.className = "custom-select-options"; list.setAttribute("role", "listbox");
    ...
    const sync = () => { const available = options(); const selected = available.find((option) => option.selected) || available[0]; value.textContent = selected?.textContent?.trim() || copy().choose; button.disabled = select.disabled; const label = select.closest("label")?.querySelector(":scope > span")?.textContent?.trim(); button.setAttribute("aria-label", select.getAttribute("aria-label") || label || selected?.textContent?.trim() || copy().choose); list.innerHTML = available.map((option, index) => `<span id="custom-option-${Math.random().toString(36).slice(2)}" role="option" data-option-index="${index}" aria-selected="${option === selected}" class="${option === selected ? "selected" : ""}">${esc(option.textContent)}</span>`).join(""); component.activeIndex = selectedIndex(); shell.classList.toggle("disabled", select.disabled); };
