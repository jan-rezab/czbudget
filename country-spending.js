      ...
      <div class="spending-chart-grid">
        ...
        <article class="spending-panel spending-change">
          ...
          <div class="delta-chart">
            ${changes.map(row => `<div class="delta-row"><strong title="${esc(row.label_native)}">${esc(row.label_native)}</strong><div class="delta-scale"><i class="zero"></i><span class="${row.delta >= 0 ? "up" : "down"}" style="width:${Math.abs(row.delta)/deltaMax*50}%;${row.delta < 0 ? "right:50%" : "left:50%"}"></span></div><b class="${row.delta >= 0 ? "positive" : "negative"}">${fmt(row.delta,c,true)}</b></div>`).join("")}
          </div>
